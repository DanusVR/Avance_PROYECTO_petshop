			...
			<div class="card-body">
				<form action="<%=url%>MascotaController" method="POST">
					<input type="hidden" name="op" value="modificar"> <input
						type="hidden" name="id_mascota"
						value="<%=mascota.getId_mascota()%>">
					...
					<div class="mb-3">
						<label for="especie" class="form-label">Especie:</label> <select
							...
							<option value="" disabled selected>Seleccione una
								especie</option>
							<option value="Perro"
								<%="Perro".equals(mascota.getEspecie()) ? "selected" : ""%>>Perro</option>
							<option value="Gato"
								<%="Gato".equals(mascota.getEspecie()) ? "selected" : ""%>>Gato</option>
						</select>
					</div>


					<div class="mb-3">
						<label for="raza" class="form-label">Raza:</label> <input
							type="text" class="form-control" name="raza" id="raza"
							value="<%=mascota.getRaza() != null ? mascota.getRaza() : ""%>">
					</div>

					<div class="mb-3">
						<label for="edad" class="form-label">Edad:</label> <input
							type="number" class="form-control" name="edad" id="edad" min="0"
					...
					<div class="mb-3">
						<label class="form-label">Sexo:</label> <select
							class="form-select" name="sexo" required>
							<option value="">Seleccione un sexo</option>
							<option value="Macho"
								<%="Macho".equals(sexoSeleccionado) ? "selected" : ""%>>Macho</option>
							<option value="Hembra"
								<%="Hembra".equals(sexoSeleccionado) ? "selected" : ""%>>Hembra</option>
						</select>
					</div>
						...
						<label for="id_cliente" class="form-label">Cliente:</label> <select
							...
							if (listarCliente != null) {
								for (Cliente cliente : listarCliente) {
									String selected = (cliente.getId_cliente() == mascota.getId_cliente()) ? "selected" : "";
							%>
