...
<div class="container py-3">
	...
	<form action="<%=url%>CitaController" method="POST"
		...
		<input type="hidden" name="op" value="modificar"> <input
			type="hidden" name="id_cita" value="<%=cita.getId_cita()%>">
		...
		<div class="mb-3">
			<label class="form-label">Estado</label> <select name="estado"
				...
				<option value="Pendiente"
					<%="Pendiente".equals(cita.getEstado()) ? "selected" : ""%>>Pendiente</option>
				<option value="Atendido"
					<%="Atendido".equals(cita.getEstado()) ? "selected" : ""%>>Atendido</option>
				<option value="Cancelado"
					<%="Cancelado".equals(cita.getEstado()) ? "selected" : ""%>>Cancelado</option>
			</select>
		</div>
		...
		<div class="mb-3">
			...
			<textarea name="observacion" id="observacion" class="form-control"
				rows="3" placeholder="Escribe una observación..."><%=cita.getObservacion() != null ? cita.getObservacion() : ""%></textarea>
		</div>

		<!-- BOTONES -->
		<button type="submit" class="btn btn-primary w-100 mb-2">💾
			Guardar cambios</button>
		<a href="<%=url%>CitaController?op=listar"
			class="btn btn-secondary w-100">↩️ Cancelar</a>
