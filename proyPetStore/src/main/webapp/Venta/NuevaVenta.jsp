		...
		<div class="card p-4">
			...
			<form action="VentaController" method="post">
				...
				<div class="row g-3">
					...
					<!-- Tipo -->
					<div class="col-md-4">
						<label class="form-label fw-bold">Tipo</label> <select name="tipo"
							id="tipo" class="form-select" onchange="mostrarCampos()">
					...
					<div class="col-md-4" id="divProd">
						<label class="form-label fw-bold">Producto</label> <select
							...
							%>
							<option value="<%=p.getId_producto()%>"><%=p.getNombre()%>
								– $<%=String.format("%.2f", p.getPrecio_venta())%></option>
							<%
							}
							}
							%>
					...
					<div class="col-md-2" id="divCant">
						<label class="form-label fw-bold">Cantidad</label> <input
							type="number" class="form-control" name="cantidad" value="1"
							min="1">
					</div>
					...
					<div class="col-md-6" id="divServ" style="display: none;">
						<label class="form-label fw-bold">Servicio</label> <select
							...
							%>
							<option value="<%=s.getId_servicio()%>"><%=s.getNombre()%>
								– $<%=String.format("%.2f", s.getPrecio())%></option>
							<%
							}
							}
							%>
				...
				<table class="table table-bordered align-middle">
					...
					<tbody>
						...
						<tr>
							...
							<td><%=d.getId_producto() != null && d.getId_producto() > 0 ? d.getNombre_producto() : d.getNombre_servicio()%></td>
			...
			<form action="VentaController" method="post">
				<input type="hidden" name="accion" value="registrar"> <input
					...
					value="<%=request.getParameter("id_Cliente") != null ? request.getParameter("id_Cliente") : ""%>">
				...
				<div class="row g-3 mt-3">
					...
					<div class="col-md-6">
						<label class="form-label fw-bold">Monto Pagado</label> <input
							type="number" step="0.01" name="monto_pagado"
				...
				<div class="text-end mt-3">
					<h4>
						Total: <span class="text-success fw-bold">$<%=String.format("%.2f", request.getAttribute("total"))%></span>
					</h4>
				</div>
